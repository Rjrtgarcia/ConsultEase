...
class FacultyStatusDiagnostic:
    ...
    def run_diagnostics(self):
        ...
        # Test 1: Database connectivity and faculty data
        self.test_database_connectivity()
        
        # Test 2: MQTT service connectivity
        self.test_mqtt_connectivity()
        
        # Test 3: Faculty controller status update functionality
        self.test_faculty_controller_updates()
        
        # Test 4: MQTT message subscription and handling
        self.test_mqtt_subscription_handling()
        
        # Test 5: End-to-end ESP32 status simulation
        self.test_esp32_status_simulation()
        
        # Test 6: Real-time dashboard update simulation
        self.test_dashboard_update_simulation()
        
        # Test 7: Database model field verification
    ...
    def test_mqtt_connectivity(self):
        ...
        try:
            if self.mqtt_service.is_connected():
                ...
                logger.warning("⚠️ MQTT service is not connected, attempting to connect...")
                # Try to connect
                time.sleep(2)
                if self.mqtt_service.is_connected():
                    logger.info("✅ MQTT service connected after retry")
                    self.test_results['mqtt_connectivity'] = 'PASS'
                else:
                    logger.error("❌ MQTT service failed to connect")
                    self.test_results['mqtt_connectivity'] = 'FAIL'
    ...
    def test_faculty_controller_updates(self):
        ...
        try:
            ...
            if result:
                ...
                logger.info(f"📊 Updated faculty data: {result}")
                
                # Wait a moment then test status change to False
                time.sleep(2)
    ...
    def print_diagnostics_summary(self):
        ...
        logger.info("   - Check faculty desk unit WiFi/MQTT connections")
        logger.info("   - Apply network fixes from FACULTY_DESK_UNIT_CONNECTION_FIX_GUIDE.md")
        logger.info("   - Verify ESP32 is publishing to correct MQTT topics")
        logger.info("   - Ensure central system MQTT router is handling faculty status properly")
